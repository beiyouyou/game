...
void LoadActor(const rapidjson::Value& actorValue, Game* game, Actor* parent)
{
	if (actorValue.IsObject())
	{
		...
		else if (type == "Player")
		{
            ...
            player->arrow = new Arrow(game, parent);
            actor = player;
		}
        else if(type == "Checkpoint"){
            Checkpoint* c;
            if(game->mChecks.size() == 0){
                c = new Checkpoint(game, parent);
                c->active = true;
                game->AddCheck(c);
            }
            else{
            ...
            if(GetStringFromJSON(actorValue, "level", levelName)){
                c->mLevelString = levelName;
            }
            std::string text;
            ...
            if (GetFloatFromJSON(actorValue, "pauseTime", f))
            {
                sc->pauseTime = f;
            }
            actor = sc;
